...
public class UIGameController : MonoBehaviour
{
	...
	public Text ScoreView; // soryung: for score presentation
    ...
    private void ShowInfoPanel(bool isShow)
    {
		...
		ScoreView.gameObject.SetActive (!isShow); // soryung: for score presentation
		TimeView.gameObject.SetActive (!isShow);
	...
	{ // soryung: for showing and hiding StartPanel presentation
		EngineForceView.gameObject.SetActive(!isShow);
		ScoreView.gameObject.SetActive (!isShow); 
		TimeView.gameObject.SetActive (!isShow); 
		RestartButton.SetActive(!isShow);
		InfoButton.SetActive(!isShow);
		ExitButton.SetActive (!isShow);
    ...
    {
		ManageScore.score [ManageScore.scoreIndex] = 0; // soryung: initialize current score
		switch(ManageScore.scoreIndex)
        {
            case 0:
	...
	{
		ManageScore.score [ManageScore.scoreIndex] = 0;
